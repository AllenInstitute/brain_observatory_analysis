...
def _default_column_names():
    ...
    """
    column_names = ['cell_specimen_id', 'cell_roi_id', 'trace', 'timepoints',
                    'oeid', 'targeted_structure', 'depth_order', 'bisect_layer']
    return column_names
...
def _default_column_base_names():
    ...
    """
    column_base_names = ['cell_specimen_id',
                         'cell_roi_id', 'trace', 'timepoints', 'oeid']
...
def _set_column_names(expt_group, column_names, column_base_names):
    ...
    column_added_names = [
        cn for cn in column_names if cn not in column_base_names]
...
def get_all_trace_df(expt_group, session_name, trace_type='dff', column_names=None):
    ...
    column_names, column_added_names = _set_column_names(
        expt_group, column_names, column_base_names)
    ...
    for oeid in oeids:
        ...
        temp_df['trace'] = trace
        temp_df['timepoints'] = [
            expt_group.experiments[oeid].ophys_timestamps] * len(csid)
        temp_df['oeid'] = oeid
...
def get_notask_trace_df(expt_group, session_name, trace_type='dff', column_names=None):
    ...
    # First experiment represents the session stimulus presentations
    stim_df = data_formatting.annotate_stimuli(
        expt_group.experiments[oeids[0]])
    ...
    for oeid in oeids:
        ...
        if first_stim_start_time > gray_period - 30:  # 30 is an arbitrary buffer
            ...
            run_pre_gray = True
            first_stim_start_frame = np.where(
                timestamps > first_stim_start_time)[0][0]
        ...
        if last_time - last_stim_end_time > gray_period - 30:  # 30 is an arbitrary buffer
            run_post_gray = True
            last_stim_end_frame = np.where(
                timestamps > last_stim_end_time)[0][0]
            ...
            if last_time - post_gray_end_time > -60:  # -60 is an arbitrary buffer
                ...
                post_gray_end_frame = np.where(
                    timestamps > post_gray_end_time)[0][0]
    ...
    # Initialize
    trace_df_gray_pre_task = pd.DataFrame(
        columns=column_names).set_index('cell_specimen_id')
    trace_df_gray_post_task = pd.DataFrame(
        columns=column_names).set_index('cell_specimen_id')
    trace_df_fingerprint = pd.DataFrame(
        columns=column_names).set_index('cell_specimen_id')
    for oeid in oeids:
        ...
        if run_pre_gray:
            if trace_type == 'dff':
                temp_df['trace'] = trace_df.dff.apply(
                    lambda x: x[:min_start_ind]).values
            else:  # Other types already dealt with from above
                temp_df['trace'] = trace_df.events.apply(
                    lambda x: x[:min_start_ind]).values
            temp_df['timepoints'] = [timestamps[:min_start_ind]] * len(csid)
            sorted_temp_df = temp_df.sort_index(inplace=False)
            trace_df_gray_pre_task = pd.concat(
                [trace_df_gray_pre_task, sorted_temp_df])
        if run_post_gray:
            if trace_type == 'dff':
                temp_df['trace'] = trace_df.dff.apply(
                    lambda x: x[max_end_ind:min_post_gray_end_ind]).values
            else:
                temp_df['trace'] = trace_df.events.apply(
                    lambda x: x[max_end_ind:min_post_gray_end_ind]).values
            temp_df['timepoints'] = [
                timestamps[max_end_ind:min_post_gray_end_ind]] * len(csid)
            sorted_temp_df = temp_df.sort_index(inplace=False)
            trace_df_gray_post_task = pd.concat(
                [trace_df_gray_post_task, sorted_temp_df])
            if run_fingerprint:
                if trace_type == 'dff':
                    temp_df['trace'] = trace_df.dff.apply(
                        lambda x: x[min_post_gray_end_ind:]).values
                else:
                    temp_df['trace'] = trace_df.events.apply(
                        lambda x: x[min_post_gray_end_ind:]).values
                temp_df['timepoints'] = [
                    timestamps[min_post_gray_end_ind:]] * len(csid)
                sorted_temp_df = temp_df.sort_index(inplace=False)
                trace_df_fingerprint = pd.concat(
                    [trace_df_fingerprint, sorted_temp_df])
...
def get_non_auto_rewarded_start_end_times(stim_df):
    ...
    if len(auto_rewarded_inds) > 0:
        ...
        for skip_ar_ind in skip_ar_inds:
            # removing those before the first auto-rewarded stim
            start_times.append(
                stim_df.iloc[auto_rewarded_inds[skip_ar_ind] + 1].start_time)
            stop_times.append(
                stim_df.iloc[auto_rewarded_inds[skip_ar_ind + 1]].stop_time)
        # When the last stimulus was not auto-rewarded, add the segments after the last auto-rewarded stim till the last stim
        if auto_rewarded_inds[-1] != len(stim_df) - 1:
            start_times.append(
                stim_df.iloc[auto_rewarded_inds[-1] + 1].start_time)
...
def get_task_trace_df(expt_group, session_name, remove_auto_rewarded=True, column_names=None):
    ...
    if remove_auto_rewarded:
        start_times, stop_times = get_non_auto_rewarded_start_end_times(
            stim_df)  # across all experiments
    else:
    ...
    max_start_inds, min_end_inds = get_start_end_inds(
        start_times, stop_times, expt_group, oeids)
    ...
    for oeid in oeids:
        ...
        try:
            test_dff_crop = np.concatenate(
                [test_dff[msi: mei] for msi, mei in zip(max_start_inds, min_end_inds)])
            timepoints = np.concatenate(
                [timestamps[msi: mei] for msi, mei in zip(max_start_inds, min_end_inds)])
            assert len(timepoints) == len(test_dff_crop)

            temp_df['cell_specimen_id'] = csid
            temp_df['cell_roi_id'] = crid
            temp_df['trace'] = dff_df.dff.apply(lambda x: np.concatenate(
                [x[msi: mei] for msi, mei in zip(max_start_inds, min_end_inds)])).values
...
def get_trace_df_stim(expt_group, session_name, stim_type, data_type='dff', image_order=3,
                      inter_image_interval=0.75, output_sampling_rate=20, remove_auto_rewarded=True, column_names=None):
    """ Get trace dataframe for a given session and stim type
    Parameters
    ----------
    expt_group: ExperimentGroup
        ExperimentGroup object
    ...
    stim_type: str
        Visual stimulus type, e.g. 'images', 'images>n-changes', 'changes', 'omissions', etc.
    data_type: str, optional
        Data type, e.g., 'dff' and 'stims', default 'dff'
    image_order: int, optional
        Image order for images>n parameters, default 3
    inter_image_interval: float, optional
        Inter image interval, default 0.75
    ...
    for oeid in oeids:
        ...
        if len(response_df) > 0:
            ...
            assert response_df.trace_timestamps.apply(
                lambda x: np.all(x == first_timestamps)).all()
            start_index = np.where(first_timestamps >= 0)[0][0]
            end_index = np.where(first_timestamps <= inter_image_interval)[
                0][-1]  # The last index is going to be not included
            ...
            for csid in csids:
                ...
                csid_trace = np.concatenate(csid_df.trace.apply(
                    lambda x: x[start_index:end_index]).values)
                trace_all.append(csid_trace)
            assert np.diff([len(trace) for trace in trace_all]).any() == False  # noqa: E712
            timepoints = np.concatenate(
                [st + tts[1:] for st, tts in zip(csid_df.start_time.values, csid_df.trace_timestamps.values)])
...
def get_all_annotated_session_response_df(expt_group, session_name, inter_image_interval=0.75, output_sampling_rate=20):
    ...
    for oeid in oeids:
        ...
        temp_df = get_stim_annotated_response_df(
            exp, stim_type='all', inter_image_interval=inter_image_interval, output_sampling_rate=output_sampling_rate)
        if temp_df is not None:
            response_df = response_df.append(temp_df)
    return response_df


def get_trace_df_stim_from_all_response_df(response_df_session, expt_group, stim_type, image_order=3, inter_image_interval=0.75):
    ...
    expt_group: Experiment group object
        Experiment group object for Learning and mFISH project
    stim_type: str
        stim type, e.g., 'images', 'images-n-omissions', 'images-n-changes', 'images>n-changes'
    image_order: int, optional
        Image order for n in stim_type, default 3

    Returns
    -------
    trace_df_stim: pandas DataFrame
    ...
    if stim_type == 'images-n-omissions':
        ...
        condition = (response_df_session['n_after_omission'] > image_order) & \
                    (response_df_session['n_after_change'] >
                     response_df_session['n_after_omission'])
    elif stim_type == 'images>n-changes':
        condition = (response_df_session['n_after_change'] > image_order) & \
        ...
        condition = (response_df_session['n_before_change'] == image_order) & \
                    (response_df_session['n_after_omission']
                     == -1)  # Get trials without omission only
    ...
    trace_df = get_trace_df_from_response_df_session(
        conditioned_df_session, expt_group, inter_image_interval=inter_image_interval)
...
def get_trace_df_from_response_df_session(response_df_session, expt_group, inter_image_interval=0.75, column_names=None):
    ...
    for oeid in oeids:
        ...
        assert response_df_oeid.trace_timestamps.apply(
            lambda x: np.all(x == first_timestamps)).all()
        start_index = np.where(first_timestamps >= 0)[0][0]
...
def get_concatenated_mean_response_df_from_response_df(response_df, csid_list=None):
    ...
    change_response_df = non_auto_rewarded_response_df.query(
        'is_change==True and auto_rewarded == False')
    change_response_df['trace_norm'] = change_response_df.apply(
        lambda x: x.trace - x.trace[0], axis=1)
    image_response_df = non_auto_rewarded_response_df.query(
        'n_after_change > 3 and (n_after_omission > 3 or n_after_omission < 0)')
    image_response_df['trace_norm'] = image_response_df.apply(
        lambda x: x.trace - x.trace[0], axis=1)
    omission_response_df = non_auto_rewarded_response_df.query('omitted==True')
    omission_response_df['trace_norm'] = omission_response_df.apply(
        lambda x: x.trace - x.trace[0], axis=1)

    cell_specimen_ids = np.sort(response_df.cell_specimen_id.unique())
    temp_df = response_df[['cell_specimen_id', 'trace']
                          ].groupby('cell_specimen_id').mean()
    assert (temp_df.index.values - cell_specimen_ids).any() == False  # noqa: E712

    trace_timestamps = response_df.trace_timestamps.values[0]
    mean_response_change_df = pd.DataFrame(
        {'cell_specimen_id': cell_specimen_ids})
    mean_response_change_df['trace'] = change_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_change_df['trace_std'] = change_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanstd(x, axis=0)).values
    mean_response_change_df['trace_norm'] = change_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_change_df['trace_std_norm'] = change_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanstd(x, axis=0)).values
    mean_response_change_df['time_stamps'] = [
        trace_timestamps] * len(mean_response_change_df)

    mean_response_image_df = pd.DataFrame(
        {'cell_specimen_id': cell_specimen_ids})
    mean_response_image_df['trace'] = image_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_image_df['trace_std'] = image_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanstd(x, axis=0)).values
    mean_response_image_df['trace_norm'] = image_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_image_df['trace_std_norm'] = image_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanstd(x, axis=0)).values

    mean_response_omission_df = pd.DataFrame(
        {'cell_specimen_id': cell_specimen_ids})
    mean_response_omission_df['trace'] = omission_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_omission_df['trace_std'] = omission_response_df[['cell_specimen_id', 'trace']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace'])).apply(lambda x: np.nanstd(x, axis=0)).values
    mean_response_omission_df['trace_norm'] = omission_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanmean(x, axis=0)).values
    mean_response_omission_df['trace_std_norm'] = omission_response_df[['cell_specimen_id', 'trace_norm']].groupby(
        'cell_specimen_id').apply(lambda x: np.vstack(x['trace_norm'])).apply(lambda x: np.nanstd(x, axis=0)).values
    ...
    concat_mean_trace = np.hstack([np.vstack(mean_response_change_df.trace.values), np.vstack(
        mean_response_image_df.trace.values), np.vstack(mean_response_omission_df.trace.values)])
    concat_mean_trace_norm = np.hstack([np.vstack(mean_response_change_df.trace_norm.values), np.vstack(
        mean_response_image_df.trace_norm.values), np.vstack(mean_response_omission_df.trace_norm.values)])
    concat_std_trace = np.hstack([np.vstack(mean_response_change_df.trace_std.values), np.vstack(
        mean_response_image_df.trace_std.values), np.vstack(mean_response_omission_df.trace_std.values)])
    concat_std_trace_norm = np.hstack([np.vstack(mean_response_change_df.trace_std_norm.values), np.vstack(
        mean_response_image_df.trace_std_norm.values), np.vstack(mean_response_omission_df.trace_std_norm.values)])

    max_timestamp = np.max(trace_timestamps)
    timestamp_interval = np.diff(trace_timestamps)[0]
    concat_timeseries = np.hstack([trace_timestamps, trace_timestamps + max_timestamp + timestamp_interval,
                                   trace_timestamps + 2 * max_timestamp + 2 * timestamp_interval])
    assert len(
        np.where(np.diff(np.unique(np.diff(concat_timeseries))) > 1e-5)[0]) == 0
